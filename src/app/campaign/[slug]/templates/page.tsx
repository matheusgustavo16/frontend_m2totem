...
import ApprovePictureComponent, {
  ApprovePictureComponentCocaCola,
  ApprovePictureComponentGarnier
} from "@/components/approvePicture";
import CountdownComponent, {
  CountdownComponentCocaCola,
  CountdownComponentGarnier
...
import {
  ...
  GetPhrases,
  GetTemplates
} from "@/app/api/services/firebase";
...
export default function PageTemplates({ params: { slug } }: PageProps) {
  ...
  const processPhoto = async (image: string, _template: any, _phrase: any) => {
    try {
      ...
      if (removed.photo) {
        ...
        // APLICA A FRASE
        try {
          const applied: any = await applyPhraseToPicture(
            removed.photo,
            _phrase.phrase,
            "vertical"
          );
        ...
        uploadString(storageRef, imageFile, "data_url")
          .then(async (data: any) => {
            ...
            const addQr: any = await AddQrPicture({
              ...
              stationId:
                localStorage.getItem("stationId") || "Não Identificada",
              campaignId: slug,
              picture: `https://firebasestorage.googleapis.com/v0/b/m2totem.appspot.com/o/${data.metadata.fullPath.replaceAll(
                "/",
                "%2F"
              )}?alt=media`
            });
            if (addQr) {
              setQrCode(
                `${window.location.origin}/campaign/${slug}/download/${addQr}`
              );
              setDownload(true);
  ...
  const applyPhrase = async (image: string, phrase: string) => {
    try {
      ...
      toast.error(
        `Falha ao aplicar a frase na imagem, tente novamente mais tarde...`
      );
      console.log("captureError", err);
    }
  };

  const onHandleApproveButton = async (
    approved: number,
    phrase?: string | null
  ) => {
    // clicou na aprovação
  ...
  return !campaignData.template_campaign ? (
    <>
      {typeof countdown === "number" && (
        <CountdownComponent seconds={countdown} onFinish={onFinishCountdown} />
      )}
      {!download && !preview && (
        <>
          <div
            className={`w-full text-center py-8 lg:px-12 flex flex-col ${
              typeof countdown !== "number"
                ? ``
                : `absolute -top-[999%] -left-[999%]`
            }`}
          >
            <div className="text-white relative bg-black my-6 min-w-[100%] min-h-[400px] m-auto rounded-xl flex justify-center items-center overflow-hidden">
              <div className="absolute w-full h-full bg-black">
                {template && template.bg && template.bg !== "" && (
                  <Image
                    src={template.bg}
                    fill
              ...
              {template && template.gabarito && template.gabarito !== "" && (
            ...
            <div className="w-full text-left flex flex-col mt-6">
              ...
              <ul className="my-4 flex gap-4">
                {templates.length >= 1 &&
                  templates.map((temp: any, k: number) => (
                    <li
                      key={k}
                      onClick={() => setTemplate(temp)}
                      className="w-64 h-28 rounded-lg bg-black cursor-pointer relative overflow-hidden"
                    >
                      <Image
                        src={`${temp.bg}`}
                        fill
          ...
          {typeof countdown !== "number" && (
            <button
              ...
              className="uppercase absolute text-white bg-[#D3303D] text-xl p-3 px-12 shadow-lg hover:shadow-xl -bottom-7 border border-white rounded-lg transition duration-300 ease-in-out font-semibold hover:bg-red-500"
            >
              capturar foto
            </button>
          )}
        ...
        <>
          <div className="pb-12 pt-8 flex flex-col justify-center items-center">
            ...
            <p className="px-32 font-semibold text-center mt-6 text-xl">
              {campaignData.phrase_download ? (
                campaignData.phrase_download.stringValue
              ) : (
                <>
          ...
          <button
            disabled
            className="uppercase absolute text-white bg-[#D3303D] text-xl p-3 px-12 shadow-lg hover:shadow-xl -bottom-7 border border-white rounded-lg transition duration-300 disabled:cursor-not-allowed disabled:bg-zinc-500 ease-in-out font-semibold hover:bg-red-500"
          >
      ...
      {!download && preview && preview !== "" && (
        <ApprovePictureComponent
          phrase_approve={
            campaignData.phrase_approve
              ? campaignData.phrase_approve.stringValue
              : null
          }
            ...
            {!template ? (
              <>
                <div className="w-full text-center flex flex-col mt-24">
                  <span className="text-5xl mb-12 w-2/3 mx-auto font-[BetterWithNarrow] tracking-wide uppercase">
                    escolha um fundo de tela
                  </span>
        ...
        <>
          <div className="mt-24 uppercase tracking-wider flex flex-col justify-center items-center gap-8">
            <div className="my-24" role="status">
              <svg
                aria-hidden="true"
                className="inline w-10 h-10 text-gray-200 animate-spin dark:text-gray-600 fill-red-100"
                viewBox="0 0 100 101"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
            ...
            <p className="w-full text-center text-5xl font-[BetterWithNarrow]">
              processando imagem e<br />
              aplicando template...
            </p>
          </div>
      ...
      {download && preview && (
        <>
          <div className="pb-24 pt-24 flex flex-col justify-center items-center">
            ...
            <div className="my-12 text-5xl font-[TCCCHolidays23Curated]">
              Viva a magia do natal!
            </div>
            <Image
              src="/assets/templates/cocacola/noel-left.png"
              width={391}
              height={490}
              alt="papai noel 2"
            ...
            <button
              disabled
              className="uppercase absolute text-white bg-[#D3303D] text-xl p-3 px-12 shadow-lg hover:shadow-xl bottom-12 border border-white rounded-lg transition duration-300 disabled:cursor-not-allowed disabled:bg-zinc-500 ease-in-out font-semibold hover:bg-red-500"
            >
    ...
    <>
      {!processing && !download && !preview && (
        <>
          <div
            className={`w-full text-center py-8 lg:px-12 pt-0 flex flex-col`}
          >
            {!template ? (
              <>
                <div className="w-full text-center flex flex-col mt-24 pb-44">
                  <span className="text-5xl mb-12 w-2/3 mx-auto font-[Gotham] font-bold text-[#ca5224] tracking-tighter uppercase">
                    escolha um
                    <br />
                    fundo de tela:
                  </span>
                    ...
                    {templates.length >= 1 &&
                      templates.map((temp: any, k: number) => (
                        <li
                          ...
                          className="w-[33%] min-h-[580px] relative"
                        >
                          <Image
                            src={`${temp.bg}`}
                            className="rounded bg-black cursor-pointer overflow-hidden border-4 hover:border-8 transition duration-300 ease-in-out border-white"
                ...
                <div className="w-full min-h-[450px] fixed left-0 bottom-0 z-0">
                  <Image
                    src="/assets/templates/garnier/product-frascos.png"
                    fill
              ...
              <>
                <div className="w-full text-center flex flex-col mt-36">
                  <ul className="my-4 max-w-3xl mx-auto grid grid-cols-1 text-center gap-4">
                    {phrases.length >= 1 &&
                      phrases.map((temp: any, k: number) => (
                        <li
                          key={k}
                          onClick={() => setPhrase(temp)}
                          className="bg-[#ca5224] p-10 px-24 text-4xl font-[Gotham] font-bold tracking-tight text-white hover:scale-105 uppercase hover:text-white transition duration-300 ease-in-out"
                        >
                          {temp.title ? temp.title : `Frase Misteriosa`}
                        </li>
                      ))}
              ...
              <>
                ...
                >
                  <div className="w-[65%] h-full mx-auto rounded-xl overflow-hidden border-2 my-20">
                    <Webcam
                      ref={videoRef}
                      audio={false}
                      // mirrored={true}
                ...
                {typeof countdown === "number" ? (
                  <>
                    ...
                    <div className="fixed w-full min-h-[480px] left-0 bottom-0 z-0">
                      <Image
                        src="/assets/templates/garnier/product-frascos-multi.png"
                        fill
                        alt="product vitamina c"
                        className="object-cover"
                      />
                  ...
                  <>
                    <div className="w-full flex justify-center items-start">
                      <button
                        onClick={activateCountdown}
                        className="bg-[#ca5224] text-white font-[Gotham] font-bold p-5 text-4xl uppercase px-12 disabled:cursor-not-allowed disabled:opacity-25 hover:scale-105"
                      >
                        Capturar Foto
                      </button>
                    </div>
        ...
        <>
          <div className="mt-24 uppercase tracking-wider flex flex-col justify-center items-center gap-8">
            <div className="my-24" role="status">
              <svg
                aria-hidden="true"
                className="inline w-10 h-10 text-zinc-100 animate-spin dark:text-gray-600 fill-zinc-400"
                viewBox="0 0 100 101"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
            ...
            <p className="w-full text-center tracking-tighter text-5xl text-[#ca5224] font-[Gotham] font-bold">
              processando imagem e<br />
              aplicando template...
            </p>
          </div>
      ...
      {download && preview && (
        <>
          <div className="pb-24 pt-24 flex flex-col justify-center items-center">
            <p className="px-32 text-center uppercase my-12 text-3xl text-[#ca5224] tracking-tighter font-[Gotham] font-bold">
              {campaignData.phrase_download ? (
                campaignData.phrase_download.stringValue
              ) : (
                <>
            ...
            <div className="relative w-80 h-80 my-12 bg-white p-4 z-20">
              {qrCode && (
                <QRCode
                  value={qrCode}
                  size={256}
            ...
            <div className="absolute w-full min-h-[480px] bottom-0 left-0 select-none z-0">
              <Image
                src="/assets/templates/garnier/bg-download.png"
                fill
                alt="garnier bottom"
                className="object-cover"
              />
            ...
            <button
              disabled
              className="uppercase absolute text-white bg-[#D3303D] text-xl p-3 px-12 shadow-lg hover:shadow-xl top-12 border border-white rounded-lg transition duration-300 disabled:cursor-not-allowed disabled:bg-zinc-500 ease-in-out font-semibold hover:bg-red-500"
            >
              redirecionando em {countdownHome || 99} segundos...
            </button>
          </div>
